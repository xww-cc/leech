...
class SingleFillTest extends TestCase
{
    /**
     * @test
     */
     ...
     * @depends opt
     */
    public function singleFill(array $opt)
    {
        $continuousFill = new SingleFill();
        $continuousFill->setOpt($opt);
        $fill_data = $continuousFill->getFillData();
        $fill_details = $continuousFill->getAfterFillDetails();
        print_r($fill_data);
        print_r($fill_details);
        $this->assertIsArray($fill_data);
